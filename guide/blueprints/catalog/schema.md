...
To add the previous Tomcat entity not only in the catalog, but in the Quick Launch as well, the entity must be tagged with `catalog_quick_launch`.
This allows admins to curate what appears in the quick launch bar on the home page. More tags can be added for the same entity, for other purposes.
...
In addition to blueprints, locations can be added to the Apache Brooklyn catalog. The example below shows a location for the vagrant configuration used in the [getting started guide]({{book.path.docs}}/start/blueprints.md), formatted as a catalog entry.
